...
/// Recursion target supplied by the dispatcher — used so a function body
/// can re-enter the dispatcher to register nested defs/classes.
...
pub(super) fn handle_function(
    ...
    if let Some(mut entity) = parse_function(node, ctx.source, ctx.path, parent_id) {
        ...
        let parent_class_name = parent_id
            .filter(|_| parent_is_class_like)
            .and_then(|pid| {
                ctx.result
                    .entities
                    .iter()
        ...
        if let Some(body) = node.child_by_field_name("body") {
            ...
            // Discover nested `def`/`class` declarations inside the
            // body. `extract_calls` deliberately skips these so the
            // outer scope doesn't claim their locals; the actual
            // registration happens here, with this function as the
            // parent. Nested-function bodies will recurse through
            // `handle_function` again, so arbitrary nesting works.
            extract_entities(body, Some(&caller_id), ctx);
        }
    }
}
