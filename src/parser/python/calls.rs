...
fn run_node_handlers(
    ...
    match node.kind() {
        ...
        _ => handle_write_forms(node, source, path, caller_id, parent_class, current_branch, result),
    }
    false
}
...
fn handle_write_forms(
    ...
    match node.kind() {
        "assignment" | "augmented_assignment" => {
            handle_self_write(node, source, caller_id, parent_class, current_branch, result);
...
fn handle_raise(
    ...
    rel.metadata.insert("raises".to_string(), "true".to_string());
...
fn handle_call(
    ...
    *call_order += 1;
    ...
    let tag_branch = |rel: &mut Relationship| {
        if let Some(b) = current_branch {
            rel.metadata
                .insert("branch".to_string(), b.to_string());
        }
    };
...
fn receiver_name(node: &Node, source: &str) -> Option<String> {
    match node.kind() {
        ...
        "parenthesized_expression" | "await" => node
            .named_child(0)
            .and_then(|c| receiver_name(&c, source)),
...
fn push_target_if_named(
    node: &Node,
    source: &str,
    splat: bool,
    out: &mut Vec<LocalTarget>,
) {
...
fn emit_local_write(
    ...
    if !already_exists {
        let span = super::super::language_parser::node_to_span(span_node);
...
fn handle_lambda(
    ...
    let span = super::super::language_parser::node_to_span(node);
