...
use super::super::helpers::parse_base_type_name;
...
pub(super) fn parse_impl(node: &Node, ctx: &mut ExtractCtx<'_>) {
    ...
    let type_name = type_node.map(|n| parse_base_type_name(&n, ctx.source));

    // Get the trait being implemented (if any)
    let trait_node = node.child_by_field_name("trait");
    let trait_name = trait_node.map(|n| parse_base_type_name(&n, ctx.source));
    ...
    if let (Some(ref type_ref), Some(ref trait_name)) = (&type_name, &trait_name) {
        let source = resolved_type_id.as_deref().unwrap_or(type_ref.as_str());
    ...
    if let Some(ref type_name) = type_name {
        ctx.impl_sources
            .push((type_name.clone(), node_text(node, ctx.source).to_string()));
    }
    ...
    let parent_id = resolved_type_id.as_deref().or(type_name.as_deref());
