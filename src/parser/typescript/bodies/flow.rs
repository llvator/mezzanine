//! Control-flow entity emission for TypeScript (TS-002): branch arms
//! (if/else-if/else, switch cases, try/catch/finally) and loop bodies
//! (`for`, `for…of`, `for…in`, `while`, `do…while`).
//!
//! Structurally identical to [`crate::parser::java::bodies::flow`] and
//! [`crate::parser::python::flow`] — the `id` / `parent_id` /
...
//! from tagged relationships and deduplicates against these. Diverging
//! here would produce two nodes per arm rather than one.
...
/// Emit a Loop entity for the body of a `for` / `for…of` / `for…in` /
/// `while` / `do…while`. Loop bodies use the `l<idx>` path prefix while
/// arms use `c<idx>`, so a sibling decision tree, loop and try chain at
/// the same scope read as `c1` / `l1` / `c2.…` rather than colliding on
/// one shared sequence.
...
/// `case_arm` tag plus, for `case <expr>:`, a `pattern:<text>` attribute
/// and matching documentation. `default:` has no pattern; the arm is still
/// emitted so the decision tree shows it was intentional.
...
/// Emit a Branch entity for one arm of a `try` statement (the try body, the
/// `catch` clause, or the `finally` clause). Mirrors the PY-001 / GR-002 /
/// Java metadata so the existing UI grouping fires without changes:
///
/// * a `try_arm` tag on every arm,
/// * a `<kind>_arm` tag (`try_body_arm`, `catch_arm`, `finally_arm`),
/// * for a `catch` that binds a parameter, the binding text travels as a
///   `caught:<name>` attribute and the entity's documentation. TypeScript
///   catches are untyped by design (`catch (e)`, or `catch (e: unknown)`
///   at most), so what is recorded is the binding rather than an exception
///   class — the Java field with the closest meaning available.
