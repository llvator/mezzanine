...
fn switch_arms_add_to_cyclomatic_but_not_to_cognitive() {
    let res = parse("function f(a: number) { switch (a) { case 1: break; case 2: break; default: break; } }");
    let (cyclomatic, _, cognitive) = metrics_of(&res, "f");
...
fn arrow_function_constants_are_measured_like_functions() {
    let res = parse("export const pick = (a: number, b: number) => { if (a) { return a; } return b; };");
...
fn an_if_else_chain_flattens_into_sibling_arms() {
    ...
    assert_eq!(names_of_kind(&res, EntityKind::Branch), vec!["c1", "c2", "c3"]);
...
fn loops_use_their_own_path_prefix() {
    ...
    let tagged = branch_tagged(&res);
    assert!(tagged.contains(&("visit".into(), "l1".into())), "{tagged:?}");
    // The `while` condition runs every iteration, so it groups with the loop.
    assert!(tagged.contains(&("ready".into(), "l2".into())), "{tagged:?}");
...
fn try_arms_are_tagged_by_kind_and_carry_the_catch_binding() {
    ...
    assert!(
        arms[1].attributes.iter().any(|a| a.starts_with("caught:err")),
        "{:?}",
        arms[1].attributes
    );
}
...
fn switch_arms_are_tagged_and_carry_their_pattern() {
    let res = parse("function f(a: string) { switch (a) { case 'x': hit(); break; default: miss(); } }");
    let arms: Vec<_> = res
        .entities
        .iter()
    ...
    assert!(
        arms[1].attributes.iter().all(|a| !a.starts_with("pattern:")),
...
fn a_receiver_less_call_stays_unqualified() {
    ...
    assert!(!targets.contains(&"Service.formatDate".to_string()), "{targets:?}");
}
...
fn a_fluent_chain_collapses_onto_its_nearest_named_object() {
    ...
    let res = parse("function run() { sel.select('c').attr('r', 1); sel.select('d').attr('r', 2); }");
    let attrs: Vec<_> = call_targets(&res).into_iter().filter(|t| t.ends_with("attr")).collect();
...
fn a_wrapped_receiver_does_not_ship_whitespace_inside_a_name() {
    ...
    let targets = call_targets(&res);
    assert!(
        targets.iter().all(|t| !t.contains('\n') && !t.contains(' ')),
        "{targets:?}"
    );
    assert!(targets.contains(&"someObject.send".to_string()), "{targets:?}");
}
...
fn a_bound_call_records_the_variable_it_binds_to() {
    ...
    let rel = res
        ...
        .expect("build edge");
    assert_eq!(rel.metadata.get("binds_to").map(String::as_str), Some("widget"));
    assert_eq!(rel.metadata.get("binds_type").map(String::as_str), Some("Widget"));
}
...
fn a_reassignment_records_rebinds_rather_than_binds() {
    ...
    let rel = res
        ...
        .find(|r| r.target_id == "build")
        .expect("build edge");
    assert_eq!(rel.metadata.get("rebinds_to").map(String::as_str), Some("w"));
...
fn a_bound_call_survives_the_stdlib_filter() {
    ...
    let res = parse("function run(xs: number[]) { const kept = xs.filter(Boolean); xs.filter(Boolean); }");
    let filters = call_targets(&res).iter().filter(|t| t.ends_with("filter")).count();
...
fn object_literal_members_carry_metrics_like_any_callable() {
    let res = parse("export const api = { pick(a: number, b: number) { if (a) { return a; } return b; } };");
    assert_eq!(metrics_of(&res, "pick"), (2, 1, 1));
    let entity = res.entities.iter().find(|e| e.name == "pick").unwrap();
    assert_eq!(entity.metrics.param_count, Some(2));
}
