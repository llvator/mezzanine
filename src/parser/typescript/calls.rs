...
use super::super::language_parser::{find_child_by_kind, node_text, ParseResult};
use super::flow::{emit_branch_entity, emit_case_arm_entity, emit_loop_entity, emit_try_arm_entity};
...
pub(super) fn extract_body_calls(
    body: &Node,
    caller_id: &str,
    caller_name: &str,
    ...
    for child in node.children(&mut cursor) {
        ...
        let binding = Binding {
            ...
            declared_type: child
                ...
                .map(|t| super::helpers::extract_type_text(&t, ctx.source)),
