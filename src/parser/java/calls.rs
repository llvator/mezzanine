...
/// Context threaded through `extract_calls` and its helpers. Groups the
/// caller identity (`caller_id`, `caller_name`, `parent_class`) with the
/// mutable walk state (`call_order`, counters, `result`) so the
/// recursive walker can pass a single reference instead of plumbing
/// nine arguments per call.
...
/// for arms of control-flow constructs. `ctx.caller_id` is the full
...
fn handle_method_invocation(
    ...
    *ctx.call_order += 1;
    if method_name == ctx.caller_name {
        return;
    }
    ...
    let receiver = node
        ...
        .map(|obj| node_text(&obj, ctx.source));
    let callee = qualify_callee(receiver, &method_name, ctx.parent_class);
    let mut rel = Relationship::new(
        ctx.caller_id.to_string(),
        callee,
        RelationshipKind::Calls,
    );
    rel.metadata.insert("order".to_string(), ctx.call_order.to_string());
...
fn apply_binding_metadata(rel: &mut Relationship, binding: Option<&Binding>) {
    ...
    let key = if b.is_reassignment { "rebinds_to" } else { "binds_to" };
...
fn handle_if_statement(node: &Node, ctx: &mut CallCtx<'_>, current_branch: Option<&str>) {
    ...
    while let Some(a) = alt {
        if a.kind() == "if_statement" {
            ...
            *ctx.arm_counter += 1;
            let path = branch_path(current_branch, *ctx.arm_counter);
            emit_branch_entity(ctx.caller_id, current_branch, &path, &a, ctx.path, ctx.result);
