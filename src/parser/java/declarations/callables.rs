...
use super::super::javadoc::extract_javadoc;
...
pub(super) fn handle_callable(
    ...
    parse_callable: fn(&Node, &str, &Path, Option<&str>) -> Option<CodeEntity>,
) {
    if let Some(entity) = parse_callable(node, ctx.source, ctx.path, parent_id) {
