...
use super::super::language_parser::{find_child_by_kind, node_text, node_to_span, ParseResult};
use super::calls::extract_calls;
use super::helpers::{extract_simple_identifier, parse_generics, parse_parameters};
...
pub(super) fn handle_function(
    ...
    if let Some(entity) = parse_function(node, source, path, parent_id) {
        ...
        let parent_class_name = parent_id.and_then(|pid| {
            result.entities.iter().find(|e| e.id == pid).map(|e| e.name.clone())
        });
