    ...
    for child in node.children(&mut cursor) {
        if child.kind() == "call_expression" {
            handle_call_expression(
                &child, source, caller_id, caller_name,
                parent_class, call_order, result,
            );
            // Continue recursing into the call_expression for nested calls
            extract_calls(&child, source, caller_id, caller_name, parent_class, call_order, result);
        } else {
            extract_calls(&child, source, caller_id, caller_name, parent_class, call_order, result);
        }
    }
...
fn extract_navigation_parts(node: &Node, source: &str) -> Option<(String, String)> {
    ...
    let method_name = {
        ...
        let found = nav_suffix.children(&mut sc)
