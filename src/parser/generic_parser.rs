...
use crate::models::{CodeEntity, EntityKind, Span, Visibility};
use anyhow::Result;
use std::path::Path;
...
impl GenericParser {
    ...
    fn try_extract_entity(&self, line: &str, line_num: usize, path: &Path) -> Option<CodeEntity> {
        ...
        match self.language {
            Language::JavaScript | Language::TypeScript => self.extract_js_entity(line, path, span),
    ...
    fn extract_js_entity(&self, line: &str, path: &Path, span: Span) -> Option<CodeEntity> {
        ...
        if line.contains("class ") {
            ...
            return Some(
                CodeEntity::new(name, EntityKind::Class, path, span)
    ...
    fn extract_java_entity(&self, line: &str, path: &Path, span: Span) -> Option<CodeEntity> {
        ...
        if line.contains(" class ") || line.starts_with("class ") {
            let name = self.extract_name_after(line, "class ")?;
            let kind = if line.contains("abstract") {
                EntityKind::AbstractClass
            } else {
                EntityKind::Class
            };
            return Some(CodeEntity::new(name, kind, path, span).with_visibility(visibility));
        }
        ...
        if line.contains(" interface ") || line.starts_with("interface ") {
            let name = self.extract_name_after(line, "interface ")?;
            return Some(
                CodeEntity::new(name, EntityKind::Interface, path, span)
        ...
        if line.contains(" enum ") || line.starts_with("enum ") {
            ...
            return Some(
                CodeEntity::new(name, EntityKind::Enum, path, span).with_visibility(visibility),
            );
        }
    ...
    fn extract_go_entity(&self, line: &str, path: &Path, span: Span) -> Option<CodeEntity> {
        ...
        if line.starts_with("func ") {
            ...
            let name = if rest.starts_with('(') {
                ...
                rest.split(')').nth(1)?.trim().split('(').next()?.trim()
            } else {
        ...
        if line.starts_with("type ") && line.contains(" struct") {
            ...
            return Some(
                CodeEntity::new(name, EntityKind::Struct, path, span).with_visibility(visibility),
            );
        }
