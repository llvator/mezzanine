...
use state::{build_config, write_json, AppState};
...
fn run_watch(opts: WatchOptions, policy: AccessPolicy, ui_dir: Option<PathBuf>) -> Result<()> {
    ...
    let config = build_config(&canonical_path, include_tests, &languages, &settings);
    ...
    let watcher_thread = spawn_file_watcher(
        ...
        tx.clone(), shared_graph.clone(), watcher_stop_rx, debounce_ms,
    );

    rt.block_on(run_http_server(
        HttpServer {
            path, output_dir, port, include_tests, languages,
            content_fallback, policy, ui_dir, allow_agent_spawn, settings,
        },
        ...
        loop {
            ...
            match notify_rx.recv_timeout(std::time::Duration::from_millis(200)) {
                Ok(Ok(events)) => {
                    ...
                    handle_reanalysis(&config, &output_dir, &graph, &tx);
                }
...
fn build_router(
    ...
    let app = Router::new()
        ...
        .route("/api/analysis/scope", post(analysis_handler::analysis_scope_handler))
