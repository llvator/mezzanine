...
f parse_store {
    d: "Content-hashed per-file parse cache on disk, and the whole of the incremental mechanism: there is no separate dirty tracking, unchanged files simply hit. One entry per file — the content hash rides inside the entry, so a re-parse overwrites rather than accumulating a new entry per edit. A file is named by the repository it belongs to plus its path within it (git's common dir, which every linked worktree shares, plus the path relative to that worktree's toplevel), falling back to the absolute path outside a repo. The absolute path alone was the original key and made 'one entry per file' mean one per spelling, so the two throwaway worktrees of a comparison shared nothing with each other or with the live tree. Naming is what makes the key an identity; it is not what makes a hit applicable, which is why reuse is a step of its own. That name decides a DIRECTORY and not only a filename: an entry lives in its repository's bucket (`r-` plus a digest of the common dir) or in `loose/` when it belongs to no repository, and each bucket carries a `_repo.json` naming itself, labelled from the common dir's PARENT so a comparison's throwaway checkout does not name the whole repository after itself. The level exists for the reader rather than the key — it is what lets the cache panel report a per-repository size from a stat-only walk, where a flat directory would mean opening all 140,987 entries to recover the same grouping."
...
fu f.parse_store.reuse {
    d: "Content decides whether an entry is stale; it does not decide which spelling it is written in. The key names a file, while everything the entry records — file_path, FileInfo::path, every entity's file_path and the ids built from them — is the path as walked, and one file has as many walked paths as there are ways to reach it: a symlinked spec directory, `mezz analyze .` against an absolute root, either side of a comparison. Serving an entry verbatim put the other spelling on every entity, so click-to-open and cr: anchors named a path the run never visited, and since content decides a hit it stayed wrong until someone edited the file; rejecting it instead, which is what usable_hit did, threw away the reuse. ParseStore::get rebases: because a ParsedFile describes one file, every path-shaped string in it is prefixed by that file's own path, so one boundary-checked substitution over the serialized tree makes the entry describe the path asked for. Field-agnostic on purpose — an enumerated list would ship a stale root the first time a path-carrying field was added. Paid only when the spelling actually differs, against an alternative of re-parsing."
    cr: "src/analyzer/parse_store.rs"
}
