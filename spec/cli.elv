...
f config {
    d: "The one knob object the CLI, the server and the MCP tools all build before analyzing: AnalysisConfig (roots, languages, spec_dir, allow_unsafe_passes), FilterConfig (include/exclude globs, test and vendor skipping) and DisplayConfig (LayoutDirection, ColorScheme, depth caps). Built per-invocation and passed down rather than read from a global, so two analyses in one process cannot contaminate each other. Settings fills what the flags left alone, from two files scoped by what they describe — the installation (ui_dir, content_fallback) or the repo (output_dir, spec_dir) — and names back every key it refuses rather than dropping it. Refusal is the point of the module: allow_agent_spawn, no_token, allow_origin and allow_unsafe_passes are not fields at either scope, and a repo-scope spec_dir may not be absolute or climb out with .., since a cloned file does not get to choose which directories mezz reads. Naming one outside the repo takes an operator, and there are three ways to be one: --spec-dir for this run, the browser UI's scope panel for this session, and a repos entry in the user file for good. That last key is the escape hatch the confinement needed — a spec in a sibling docs repo is a real layout, and mezz watch, mezz mcp and the VS Code extension are all launched with no flag anyone could add. repos holds repo-scope settings keyed by a checkout's path, in the user's own file, and it outranks that checkout's own file: naming a repo by path is the reader speaking about that repo, not a machine-wide default a more specific file should beat. It is REJECTED in a repo file rather than merely ignored, a cloned block being a wider grant than the spec_dir escape already refused — it would pick what mezz reads for every checkout on the machine. The module is a folder: mod.rs loads and refuses, report.rs answers which link of the chain won."
...
fu f.config.refuse {
    d: "The refusals became data without ceasing to be stderr. Every diagnostic was an eprintln! and nothing else, which is right for a terminal and useless for a browser: a typo'd exclude_pattern did nothing visible, and neither did a cloned repo asking for allow_agent_spawn — the case the loudness was for. read now returns (Settings, Vec<Warning>) and load_scoped prints them, so the two surfaces cannot drift and a terminal user's output is unchanged. Each Warning carries the file, the key and a Severity: Rejected for a REJECTED key named with the capability it would have granted, and for a repos block in a repo-scope file, Ignored for a typo or a wrong-scope key or a spec_dir that climbs out or a repos block nested inside a repos entry, Malformed for a file that would not parse — the last being the one most easily missed, since every value the reader believes they set is then quietly a default. misplaced folds report_repo_only and clear_user_only into one loop over data, and clear_misscoped holds the per-scope rules so validate itself stays branchless. from_entry prefixes an entry's diagnostics with the repos key they came from and leaves the setting name bare, since that name is what the browser panel matches on to sit a warning beside its row. unknown_languages is recomputed rather than captured, apply_filters running once per command and long after the file was read. Nothing here may fail a command that would otherwise work."
