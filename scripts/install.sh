#!/usr/bin/env bash
# Build and install Nao from this checkout: both binaries (nao, elevator)
...
#
# Env overrides:
#   NAO_CODE_CLI=cursor   # or code-insiders, codium — defaults to the first found
#   NAO_SKIP_EXTENSION=1  # skip the VS Code extension
#   NAO_SKIP_UI=1         # skip the browser UI (no node toolchain needed)
set -euo pipefail
...
echo "==> Installing nao + elevator binaries to $CARGO_BIN"
cargo install --path "$REPO_ROOT" --force

# The browser UI `nao watch` serves.
#
...
# installed nao could only ever serve a UI when launched from a repo root,
...
# checkout moves, and would make an installed nao depend on the repo it came
# from.
#
# A UI stays optional throughout: with none installed, `nao watch` still runs,
...
if [[ -n "${NAO_SKIP_UI:-}" ]]; then
  echo "==> NAO_SKIP_UI set — not installing the browser UI."
  ...
  echo "   The binaries are installed; \`nao watch\` will explain how to connect one." >&2
else
...
if [[ -z "$CODE_CLI" ]]; then
  ...
  echo "   (or set NAO_CODE_CLI=<your-cli>, or NAO_SKIP_EXTENSION=1)" >&2
  exit 1
fi
